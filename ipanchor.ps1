...
$Dir = "$env:ProgramData\IPanchor"   # per mode (local, virtual): current.MODE.txt = applied now, saved.MODE.txt = restored on every boot
...
function State($kind, $m) { "$Dir\$kind.$m.txt" }   # kind: current or saved; line: MODE|IF|IP|MASK|GW|DNS|DNS2
...
function Undo($m) {  # take back what mode $m has applied (local goes back to DHCP)
  $f = State current $m   # not $c: PowerShell names ignore case and $C holds the config
  if (-not (Test-Path $f)) { return }
  $o = (Get-Content $f -Raw).Trim() -split '\|'; Remove-Item $f
  if ($m -eq 'virtual') {
...
function Apply {  # bring $C.MODE up; the other mode is not touched
  New-Item -ItemType Directory -Force $Dir | Out-Null
  $f = State current $C.MODE
  if (Test-Path $f) {   # a new local config on the same interface replaces the old one in place
    $old = (Get-Content $f -Raw).Trim(); $o = $old -split '\|'
    if (-not (($o[0] -eq 'local' -and $o[1] -eq $C.IF) -or $old -eq (Line))) { Undo $C.MODE }
  }
  Line | Set-Content $f -Encoding UTF8
  if ($C.MODE -eq 'local') { UpLocal } else { UpVirtual }
}

function Restore($m) {  # back to the saved config of mode $m, or take it away when nothing is saved
  if (Test-Path (State saved $m)) { Load (State saved $m); Apply } else { Undo $m }
}

function Cleanup {  # nothing applied or saved in any mode: the boot task is not needed any more
  if (-not (Get-ChildItem "$Dir\current.*.txt", "$Dir\saved.*.txt" -ErrorAction SilentlyContinue)) { BootOff }
}
...
if (-not $me.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {   # ask for Administrator and rerun there
  $a = '-NoExit', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', "`"$PSCommandPath`""
  if ($Boot) { $a += '-Boot' }
  try { Start-Process powershell.exe -Verb RunAs -ArgumentList $a } catch { Fail 'run it as Administrator' }
  exit
}
if ($Boot) {   # one mode failing must not stop the other
  foreach ($m in 'virtual', 'local') { try { Restore $m } catch { Add-Content "$Dir\boot.log" "$(Get-Date -Format s) $m $_" } }
  Cleanup; exit
}

$SysName = (Get-CimInstance Win32_OperatingSystem).Caption
while ($true) {
  Say '' ' ----------------------' '  Welcome to IPanchor' ' \--------------------/' "  $SysName" '' '  1 > set on virtual' '  2 > set on local network' '  3 > reset on virtual' '  4 > reset on local network' '  5 > reset all' '  6 > quit' ''
  $choice = Read-Host '>'
  if ($choice -eq '1') { $C.MODE = 'virtual' }
  elseif ($choice -eq '2') { $C.MODE = 'local' }
  elseif ($choice -in '3', '4', '5') {
    foreach ($m in @{ '3' = @('virtual'); '4' = @('local'); '5' = @('virtual', 'local') }[$choice]) {
      if (-not (Test-Path (State current $m))) { Say "Nothing applied on $m." }
      Undo $m; Remove-Item (State saved $m) -ErrorAction SilentlyContinue
    }
    Cleanup; Final
  }
  elseif ($choice -in '6', 'q') { exit }
  else { continue }
  ...
  if (-not $ok) { Fail "$why, restoring the previous config"; try { Restore $C.MODE; Cleanup } catch { Fail $_.Exception.Message }; continue }
  if ($keep) { Copy-Item (State current $C.MODE) (State saved $C.MODE) -Force }
  BootOn
