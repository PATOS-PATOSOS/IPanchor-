...
DIR=/etc/ipanchor
CUR=$DIR/current   # config applied now: MODE IF IP MASK GW DNS DNS2
SAVED=$DIR/saved   # config restored on every boot (no file = back to DHCP)
...
apply() {
  mkdir -p $DIR
  if [ -f $CUR ]; then
    read -r om oif x < $CUR   # a new local config on the same interface replaces the old one in place
    [ "$om $oif" = "local $IF" ] || [ "$(cat $CUR)" = "$(line)" ] || undo
  fi
  line > $CUR
  up_$MODE
}

undo() {  # take back what CUR says is applied (local goes back to DHCP)
  [ -f $CUR ] || return 0
  read -r om oif x < $CUR; rm -f $CUR
  if [ "$om" = virtual ]; then undo_virtual; else undo_local "$oif"; fi
  return 0
}

restore() {  # back to the saved config, or to DHCP when nothing is saved
  if [ -f $SAVED ]; then read -r MODE IF IP MASK GW DNS DNS2 < $SAVED; P=$(mask2cidr "$MASK"); apply
  else undo; boot_off; fi
}
...
[ "$1" = boot ] && { restore; exit 0; }
[ "$(id -u)" = 0 ] || { say "Run it as root: sudo sh $0"; exit 1; }
[ -f "$0" ] || { say "Download ipanchor.sh and run that file (the boot service needs it): sudo sh ipanchor.sh"; exit 1; }

while :; do
  say "" " ----------------------" "  Welcome to IPanchor" " \\--------------------/" "  ${SYSNAME:-$(uname -s)}" "" \
    "  1 > set virtual" "  2 > set on local network" "  3 > Reset config" "  q > quit" ""
  ask ">"
  case $A in
  1) MODE=virtual ;;
  2) MODE=local ;;
  3) [ -f $CUR ] || say "IPanchor has nothing applied, nothing to reset."
     undo; rm -rf $DIR; boot_off; final ;;
  q | Q) exit 0 ;;
  *) continue ;;
  esac
  netconfig || continue
  ...
  if ! apply; then fail "the config could not be applied, restoring the previous one"; restore; continue; fi
  if ! verify; then fail "gateway $GW does not answer from $IP/$P, restoring the previous one"; restore; continue; fi
  [ -n "$keep" ] && cp $CUR $SAVED
  boot_on
